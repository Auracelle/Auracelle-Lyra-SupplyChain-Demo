...
### Bug Fixes
- Fixed `Unexpected token ,` SyntaxError caused by double comma in P3_STARTERS.Nuclear
  block — this was preventing the entire P3 wargaming engine from loading,
  causing `p3SetDomain is not defined` errors when switching domain tabs
- Removed State Formula card (bgc_to_state() mapping panel) from Game State tab
  — IP exposure: STRATUM II BGC → state vector formula removed from public-facing view
